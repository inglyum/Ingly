...
// Aggrega i KPI CRM+catalogo per la dashboard. Ritorna sempre una struttura
// completa (0 se vuoto), mai eccezioni verso l'alto: errori → metriche a 0.
export async function loadDashboard(sb) {
  ...
  try {
    const [customers, companies, activities, products] = await Promise.all([
